# Run fix script on Snapmaker U1 via plink
# Usage: .\run-fix.ps1
# This script pipes fix_rc_loc.sh content through plink to run on the U1
...
$content = [System.IO.File]::ReadAllText('fix_rc_loc.sh')
$content = $content.Replace("`r`n", "`n")

Write-Host "=== Running fix_rc_loc.sh on $SSH_USER@$TARGET_IP ==="
